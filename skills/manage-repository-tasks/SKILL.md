...
The collapsible graph appears after the task cards and has no vertical height limit or nested vertical scrollbar; it grows downward for as long as the diagram requires. The graph button beside the repository heading expands the graph, scrolls to it, moves keyboard focus there, and updates the report address to its stable `#graph` fragment. A report can therefore be opened or shared directly at the graph as `file:///c:/path/to/report.html#graph`. The graph has two views. Blocking flow uses a left-to-right layout for tasks connected by BLOCKS relationships. Tasks without blocking links in the included set appear in a compact list below it. Task families shows every included task in an indented parent tree without dependency arrows. Keeping these structures separate prevents cross-family dependencies from stretching or overlapping nested family boxes. Labels retain their natural size; wide flows scroll horizontally instead of shrinking text.

Both graph views link to task cards, including cards outside the active dashboard filters, and IDs are bold. Family references include the task type, status, and priority. Flow nodes use GATE diamonds, BUG hexagons, and FEATURE/TASK rounded rectangles. Ready, blocked, and recently done tasks use green, red, and gray styling respectively. Arrows represent only explicit BLOCKS relationships, including retained links on recently done tasks; they do not imply that completed work still blocks execution. Parent relationships remain in Task families, and hovering a flow node shows its direct parent. A newly focused card highlights through a `#FFFF50` fill that fades over 4.8 seconds before returning to its normal status styling. The Mermaid renderer is bundled so the report stays self-contained and offline.
